...
final class WaveletsTests: XCTestCase {
    ...
    func testWavelets() {
        ...
        let actualWaveletTransformation = wavelets.applyStationaryWaveletsTransformation(signal: inputSignal, wavelet: "db3", level: 3)
        let actualDetailCoeff = actualWaveletTransformation.getDeteailCoefficientAt(level: 3)
        ...
        // Only verifying the detail coefficient from the latest level since that are coefficient of interest (by kali algortihm)
